...
| D-023 | เปลี่ยนขอบเขต BL-02 จาก "LLM recipe decomposition" เป็น **AI Import** + เลื่อนจาก P5 → P4 (research gate แบบเบา — เกณฑ์สำเร็จ/ground truth ตอบได้จาก pipeline จริงที่วีใช้อยู่แล้ว ไม่ต้องทำ research doc ยาวแบบเต็ม D-013): user ทุกคน (ไม่ใช่แค่ admin) ส่งชื่อ+ภาพ+ปริมาณ → Claude ผ่าน proxy (key เดียวกับ D-015) → คืน JSON ตาม schema เดียวกับ admin bulk-import เป๊ะ (ตัดค่า 0 ยกเว้น kcal/protein/carbs/fat) → หน้า preview ให้ user แก้ทุกค่าก่อน save → save ผ่านเส้นทาง create custom food ปกติ (creator = user คนนั้น, verification badge ตาม D-017 ปกติ). แยกชัดจาก admin bulk-import ซึ่งยังเป็น admin-only เหมือนเดิม ไม่เปิดกว้างขึ้น — AI Import คือตัวช่วยกรอกฟอร์มทีละรายการ ไม่ใช่ bulk write. หลักการบังคับ: LLM ไม่มีสิทธิ์เขียน DB ตรงในทุกกรณี ทำได้แค่ pre-fill ฟอร์ม — human verification โดยผู้สร้าง (preview+แก้เอง) เป็นขั้นตอนข้ามไม่ได้ ตามด้วย admin verification (D-017) เป็นชั้นที่สอง — **ยกเลิก (2026-08-20)**: implement + validate ครบ
3 รอบแล้ว (research doc, code, ปรับ metric ตามคำท้วง, เพิ่ม confidence range + read_label mode + cooking
method context) ผลวัดสรุป: error median kcal ~70-90 ต่อรายการ (ยอมรับได้ในตัว), แต่ **confidence range
coverage ต่ำกว่าเป้า 80% มาก (26-40%, มั่นใจเกินจริง)** และ **cooking-method context ทำให้คู่อ้างอิงหลัก
(ทอด vs จี่) ห่างจาก ground truth มากขึ้นแทนที่จะแม่นขึ้น** (127.5kcal gap vs ground truth 25kcal) — สรุป
คือ feature ไม่แก้ปัญหาที่ตั้งใจจริง (user ยังต้องมีความรู้พอตรวจเอง = วนกลับมาที่วี) แถมมีต้นทุน API
credit เพิ่มจากที่ไม่มีอยู่แล้ว โดย pipeline ChatGPT+ตรวจเอง+admin bulk-import ที่วีใช้อยู่ยังคงดีกว่าและ
ถูกกว่า — ปิด `AI_IMPORT_ENABLED` ถาวร, โค้ด+validation script เก็บไว้ (ต้นทุน maintain ~0), research doc
...
จริง: Diary · Search · Dashboard · Friends · Settings** — แทรก Friends ก่อน Settings เท่านั้น ไม่ย้าย 4 ปุ่ม
เดิม (ต้นทุนย้ายปุ่มที่คนใช้จริง 3 คนคุ้นมือแล้วสูงกว่าแทรกปุ่มใหม่ก่อน Settings มาก, order เดิมที่เขียนไว้
ตอน BL-08 ยังไม่มีจริง เป็นแค่การเดาบนกระดาษ) — **ส่วนที่ 2 ยังรอเกณฑ์เดิม**: วัดจำนวนคำขอ BL-12 จากเพื่อน
...
`logSearchLatency()`, `SEARCH_LATENCY_LOGGING` flag ออกจาก `FoodSearch.tsx`/`config.ts`, drop ตาราง
`search_latency_log` (migration `20260820000000_drop_search_latency_log.sql`) — **BL-11 ปิดสมบูรณ์** |
| BL-12 | **เสร็จแล้ว — FR-FOOD-9, P4c, tag `v1.4.0`** | | **ขออาหารใหม่ในแอป** (2026-08-20, ทดแทน D-023 ที่ยกเลิก) — เพื่อนค้นหาแล้ว
ไม่เจอ กดปุ่ม "ขอเพิ่ม" ในแอป: กรอกชื่อ + ปริมาณ + แนบรูป (optional) → เข้าคิวรวมที่เดียวให้วีเห็น →
...
สะสม |
| BL-14 | **เสร็จแล้ว 2026-08-21** — implement เป็น FR-FRIEND-1 รวมกับ BL-09 ส่วนที่ 1 | | **In-app
...
หนึ่งพอ):** (1) ตัดสินใจทำ meal reminder จริงจาก BL-10 — ตอนนั้นต้องเทียบ 2 ทาง: Web Push vs iOS Shortcuts
Automation (ใช้ per-user token ที่มีอยู่แล้วจาก D-020 ไม่ต้องมี service worker/VAPID/subscription table
เลย) (2) มี event ใหม่ในระบบที่ "รอเปิดแอปไม่ได้" เกิดขึ้นจริง (3) เพื่อนบ่นเองว่าพลาดของสำคัญเพราะไม่มี
แจ้งเตือน — **ข้อจำกัดทางเทคนิคที่สรุปไว้แล้ว (กันต้อง research ซ้ำตอนกลับมาทำ):** iOS รองรับ Web Push
จริงตั้งแต่ 16.4 (2023) แต่เฉพาะ PWA ที่ "เพิ่มไปหน้าจอโฮม" แล้วเท่านั้น (ไม่ใช่เปิดใน Safari tab เฉยๆ),
...
admin) ทุกคนเข้าถึงได้ — แสดงชื่อผู้เขียน (nickname ตาม D-016) ทุกโพสต์ — เป็นแหล่งที่ 4 ใน feed ที่มีอยู่แล้ว
reuse โครงเดิมทั้งหมด (badge/`feed_last_seen_at` cursor) โพสต์ใหม่ทำให้ badge ขึ้นเหมือนแหล่งอื่น — ยังไม่มี
comment/reaction (แยกเป็น **BL-17** รอเกณฑ์ BL-09 ส่วนที่ 2) — ไม่มี push ตามหลักการเดิม — **เหตุผล:** ปิด
ช่องว่างที่เจอซ้ำสองรอบแล้วคือระบบยังไม่มีช่องทางสื่อสารทิศทางวี→ทุกคน (BL-12 แก้ทิศทางเพื่อน→วี, โน้ต admin
ใน BL-12 แก้เฉพาะรายคำขอ) — เปิดให้ทุกคนโพสต์ (ไม่ใช่ admin-only) เพราะตารางเดียวกันต่างแค่ RLS policy
แยกทำทีหลังคือรื้อ policy+UI รอบสอง และไม่มีข้อเสีย (ไม่มีใครโพสต์ feed ก็ยังมีของจากอาหาร/จานใหม่อยู่ดี,
วีโพสต์ก่อนเป็นการตั้งตัวอย่างแก้ cold start เอง) — ทำหลัง FR-FRIEND-1 dogfood ผ่านแล้ว (เสร็จแล้ว) |
| BL-17 | รอเกณฑ์ BL-09 ส่วนที่ 2 เดียวกัน (2026-08-22) | | Comment/reaction บนโพสต์ (BL-16) — แยกออกจาก
BL-16 ตั้งใจ เพื่อไม่ให้ BL-16 บวมจนต้องรอเกณฑ์ไปด้วย — ประเมินพร้อม BL-09 ส่วนที่ 2 วันที่ 2026-09-19 |
| BL-18 | **ยกเลิก 2026-08-22** (คำสั่งวี หลัง discuss กับที่ปรึกษา) | | ~~รูปภาพในโพสต์ + ย้ายไป Supabase
Storage~~ — เหตุผลยกเลิก: base64 ใน DB (pattern เดียวกับ `food_requests`) จะกิน space มากเพราะโพสต์อยู่
ถาวรไม่มีวันลบเหมือนคำขออาหาร (~300-700KB/แถว จะกิน free tier 500MB เร็ว) และทำ query feed ช้าลงเรื่อยๆ —
ทางที่ถูกคือ Supabase Storage (free 1GB แยกจาก DB + CDN, DB เก็บแค่ URL) แต่เป็น infra ใหม่ที่ต้องจัดการ
...
| BL-21 | **เสร็จแล้ว — FR-BADGE-1, P4f, tag `v1.7.0`** | | **Achievement badges** สไตล์ Apple Fitness (ล็อก =
...
(ทั้งที่ได้แล้ว+เงา) — **เลือก badge ที่โชว์บนการ์ด: ผสม "เพิ่งได้ล่าสุด" กับ "ใกล้จะได้" (เช่น สร้างอาหาร
ครบ 10 — ตอนนี้ 8/10)** ไม่ใช่เอาที่ได้แล้วมาเรียงอย่างเดียว (ของที่ได้แล้วไม่สร้างแรงจูงใจ ของที่ใกล้ได้คือ
สิ่งที่ทำให้อยากทำต่อ) — badge ใกล้ได้ต้องโชว์ความคืบหน้า (8/10)

**หลักบังคับ:** badge เป็นของส่วนตัว ห้ามแสดงยอด/เรียงเทียบกับคนอื่นทุกกรณี (leaderboard ทางอ้อม แบนไว้
ตั้งแต่ BL-09) — ปลดล็อกขึ้น Friends feed เป็น event ใหม่ 1 ครั้ง ("X ปลดล็อก: {ชื่อ}") ใช้ toggle แชร์เดิม
ของ FR-FRIEND-3 — ใช้ยอดสะสมเป็นหลัก ไม่ใช้ streak เพิ่ม (streak ขาดแล้วศูนย์ทำให้เลิกใช้ + เสี่ยงดันให้ฝืน
บันทึกผิดความจริงในแอปอาหาร ให้ streak มีแค่กลุ่มเดียวใน FR-FRIEND-3) — ห้ามมี badge เกี่ยวกับ kcal/deficit/
น้ำหนักลด (รางวัลสำหรับกินน้อยเป็นกลไกอันตราย) — badge ที่ล็อกอยู่ต้องมีคำอธิบายเงื่อนไขชัดเจน ไม่ใช่ปริศนา

**ชุดเริ่มต้น 4 กลุ่ม:** สม่ำเสมอ (ก้าวแรก/สัปดาห์ครบถ้วน/เดือนครบถ้วน/ไม่ขาดสาย 7-30-100 วัน) — ผู้สร้าง
(เชฟฝึกหัด/นักสะสมสูตร 10,50/ตรารับรอง/ของดีบอกต่อ 10,100/นักปรุง จาน 1,10) — โภชนาการ (โปรตีนครบสะสม
7,30 วัน) — การเดินทาง (หนึ่งเดือน/ร้อยวัน/หนึ่งปี ใช้แอปครบ 30/100/365 วัน, ขาประจำตาชั่ง น้ำหนัก 10,50 ครั้ง)

**ยังไม่ตัดสิน — รอร่าง FR:** วิธีคำนวณสถานะ badge (นับสดทุกครั้งที่เปิดหน้า vs เก็บตาราง `user_badges`
ตอนปลดล็อก) พร้อม trade-off, และแหล่งข้อมูล progress (8/10) โดยไม่ยิง query หนักทุกครั้งที่เปิด Dashboard |
...
- v1.37 (2026-08-22): ยกเลิก **BL-18** (รูปในโพสต์/Storage, คำสั่งวี หลัง discuss ที่ปรึกษา — pattern
  เดียวกับ D-023) ใช้ auto-link แทน, เพิ่ม **BL-19** (link preview, ยังไม่ทำ), เพิ่ม **BL-20** (activity
  events ส่วนที่เหลือหลังแยก 4 event แรกไป FR-FRIEND-3)
- v1.36 (2026-08-22): แก้สถานะ backlog ที่ค้างเก่า (พบตอนวีถามจำนวน BL ทั้งหมด) — BL-01/BL-05/BL-08/BL-10/
  BL-12 status column ไม่เคยอัปเดตหลังชิปจริงเป็น FR-DIARY-4/FR-CALC-4/FR-DASH-1/FR-CALC-5/FR-FOOD-9
  ตามลำดับ — mark เสร็จแล้วพร้อมอ้าง FR+tag ให้ตรงของจริงทั้งหมด
...
- v1.33 (2026-08-22): ปิด P4d (tag `v1.5.0`, คำสั่งวี) — BL-09 ส่วนที่ 1 เสร็จ (dogfood ผ่านครบ 7 ข้อ),
  ส่วนที่ 2 ยังรอเกณฑ์เดิม (นับจากวันที่วีแจ้งเพื่อน ยังไม่ระบุวันที่)
- v1.32 (2026-08-22): ปิด BL-14 (เสร็จแล้วผ่าน FR-FRIEND-1) — เพิ่ม **BL-15** (Web Push notification,
  คำสั่งวี) ยังไม่ทำ พร้อมเงื่อนไขกลับมาพิจารณา 3 ข้อ + ข้อจำกัดทางเทคนิคที่ research ไว้แล้ว (iOS 16.4+
  ต้อง add-to-home-screen, ต้องมี service worker/VAPID/subscription table/push server ใหม่ทั้งหมด)
- v1.31 (2026-08-21): BL-09 ส่วนที่ 1 เริ่มทำเป็น FR-FRIEND-1 (แท็บ Friends แยก ไม่ใช่ Dashboard section,
  คำสั่งวี) — ยืนยัน toolbar 5 ปุ่มเป็นของจริง (Diary·Search·Dashboard·Friends·Settings), แทรก Friends
  ก่อน Settings ไม่ย้ายปุ่มเดิม
- v1.30 (2026-08-21): ปิด P4c (tag `v1.4.0`, คำสั่งวี) — วี dogfood BL-12/BL-01 ผ่านครบ 8 ข้อ — เพิ่ม
  **BL-14** (in-app notification badge, ทำเป็นงานถัดไป) — **ปรับขอบเขต BL-09** จาก "Friends tab" เดี่ยวๆ
  เป็น "ข่าวสาร + สังคม" 2 ส่วน (feed ข่าวสารทำได้เลย, leaderboard/contribution/activity feed รอเกณฑ์เดิม)
- v1.29 (2026-08-21): ปิด P4b (tag `v1.3.0`) — BL-09 เลื่อนอีกครั้งพร้อมเกณฑ์ตัดสินผูกกับผลใช้งานจริงของ
  BL-12 (คำสั่งวี), BL-12 กลายเป็นงานถัดไป (ร่าง FR รอส่งอนุมัติ), BL-01 ยืนยันยังต้องการอยู่ ทำรวมรอบ
  เดียวกับ BL-12
...
- v1.26 (2026-08-20): **D-023 (AI Import) ยกเลิกถาวร** — วีตัดสินใจหลังผลวัด round 3 (coverage มั่นใจ
  เกินจริง, cooking-method context ทำให้แย่ลง) ไม่คุ้มต้นทุน API เทียบกับ pipeline ChatGPT+ตรวจเองที่ใช้
  อยู่ — ปิด `AI_IMPORT_ENABLED` ถาวร, โค้ด+validation script+research doc เก็บไว้ทั้งหมด (ไม่ลบ) — เพิ่ม
  BL-12 (ขออาหารใหม่ในแอป) ทดแทน แก้ปัญหาการประสานงานแทนที่จะพยายามแทนที่ความรู้วีด้วย AI — P4a เหลือ BL-08
  เป็นงานถัดไป
