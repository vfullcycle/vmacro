...
1. เปิดแอป **Shortcuts** บน iPhone → กด **+** (New Shortcut) → ตั้งชื่อ **`Vmacro: Sync to Health`
   เป๊ะๆ** (ตัวพิมพ์ใหญ่เล็ก/เว้นวรรคต้องตรงทุกตัวอักษร) — ปุ่ม "ซิงก์เข้า Apple Health" ในหน้า Diary ของเว็บแอป
   เปิด Shortcut ด้วยชื่อนี้ตรงๆ ผ่าน deep link `shortcuts://run-shortcut?name=...` ถ้าตั้งชื่ออื่นปุ่มนั้นจะหา
   Shortcut ไม่เจอ (แต่ยังรันเองจากในแอป Shortcuts ได้ปกติ ไม่กระทบอะไร)
...
เพื่อ sync ทันทีโดยไม่ต้องรอ automation — กดแล้วเปิดแอป Shortcuts มารัน Shortcut #1 ให้เลย (ต้องตั้งชื่อ
Shortcut ตรงตามข้อ 1 ด้านบน)

**ข้อควรระวัง: รันซ้ำในวันเดียวกันจะได้ยอดเบิ้ล** — `Log Nutrition Sample` สร้าง entry ใหม่ทุกครั้งที่รัน
ไม่ได้ overwrite ของเดิม เพราะฉะนั้นถ้ากดปุ่มนี้หลังทุกมื้อ ยอดใน Health จะสะสมเกินจริง (เช่น กด 3 รอบ/วัน
kcal ใน Health จะโชว์ ~3 เท่าของความจริง) เวอร์ชันนี้ยังไม่มีการลบ sample เก่าก่อนเขียนใหม่ให้อัตโนมัติ —
**แนะนำ sync แค่ครั้งเดียวต่อวัน** (เช่น ก่อนนอน ตอนล็อกมื้อสุดท้ายแล้ว) ถ้าอยากกดได้หลายรอบต่อวันโดยไม่เบิ้ล
ต้องเพิ่ม action `Find Health Samples` + `Delete Health Sample` ลบของเก่าของวันนั้นก่อน log ใหม่ทุกครั้ง —
ยังไม่ได้ระบุไว้ในคู่มือนี้ บอกวีได้ถ้าต้องการให้เพิ่ม
...
ปิด "Ask Before Running" ถ้าอยากให้รันเงียบๆ ไม่ขึ้น prompt — **อย่าตั้งทั้ง automation รายวันและกดปุ่ม sync
เองในวันเดียวกัน** เพราะจะเจอปัญหายอดเบิ้ลข้างต้นเช่นกัน
