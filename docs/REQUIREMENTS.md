...
**FR-DASH-2** **[SUPERSEDED 2026-08-21 โดย FR-FRIEND-1 — ดูด้านล่าง]** เดิมวางแผนให้ feed +
notification badge อยู่เป็น section ในหน้า Dashboard — วีตัดสินใจย้ายออกเป็นแท็บ "Friends" แยกต่างหากตั้งแต่
รอบแรกแทน เพราะ BL-09 บันทึกไว้แล้วว่าปลายทาง toolbar คือ 5 ปุ่มรวม Friends อยู่แล้ว การแปะไว้ใน Dashboard
ก่อนแล้วย้ายทีหลังคือทำงานสองรอบ + ย้ายบ้านของ user โดยไม่จำเป็น — โค้ดที่ deploy ไปแล้วตาม spec นี้ (รวม RPC
`get_new_admin_foods`) ถูกรื้อ/แทนที่ทั้งหมดตาม FR-FRIEND-1

## FR-FRIEND — Friends tab (BL-09)

**FR-FRIEND-1 (ยังไม่กำหนด phase)** Friends tab เริ่มจาก feed + notification badge (BL-14 + BL-09
ส่วนที่ 1) — เนื้อหาเชิงสังคม (leaderboard, contribution board, activity feed — BL-09 ส่วนที่ 2) เติมเข้า
แท็บเดียวกันทีหลังตามเกณฑ์ที่บันทึกไว้ใน backlog ไม่กระทบ navigation อีก — **Toolbar ใหม่ 5 ปุ่ม: Diary ·
Search · Dashboard · Friends · Settings** (แทรก Friends ก่อน Settings เท่านั้น ไม่ย้าย 4 ปุ่มเดิมที่มีคนใช้
จริง 3 คนคุ้นมือแล้ว — ต่างจาก order ที่เคย sketch ไว้ตอน BL-08 ยังไม่มีจริง)

Profile เพิ่ม `feed_last_seen_at` (default `now()` ตอน migrate/สมัครใหม่ กันของเก่าทั้งหมดโผล่เป็น "ใหม่"
พรวดเดียวตอน launch) → **badge จุดแดงบนไอคอนแท็บ Friends** เช็คตอนเปิดแอปเท่านั้น (ไม่ push): มีอะไร
`created_at`/`updated_at` ใหม่กว่า `feed_last_seen_at` ใน 3 แหล่งนี้ไหม — (1) **อาหารใหม่ทั้งหมด** จาก
ทุกคน (`custom_foods`, อ่านได้ทุกคนอยู่แล้วตาม FR-FOOD-2 RLS — ไม่ต้องแยก admin/คนอื่นเป็นคนละ query
เพราะแสดงชื่อผู้สร้างต่อท้ายอยู่แล้ว แยกแยะได้เองจากชื่อ) (2) จานใหม่จาก **ทุกคน** (`dishes`) (3) คำขอของ
ตัวเอง (`food_requests`) ที่ถูกตอบแล้ว (fulfilled/declined พร้อมโน้ต admin ถ้ามี) — **ทุกรายการอาหาร/จาน
แสดงชื่อผู้สร้าง (nickname ตาม D-016) เสมอ ไม่ซ่อน** (สอดคล้องกับ FR-FOOD-2 ที่แสดงชื่อ creator ในผลค้นหา
อยู่แล้ว, ไม่มีประเด็น privacy เพราะ custom food/จานเป็นข้อมูลกลางที่ทุกคนค้นเจออยู่แล้วตาม D-002) —
**ไม่รวม "อาหารที่เพิ่ง verify"** เป็นแหล่งที่ 4 ตามที่เสนอไว้ตอนแรก (ตัดออก — ซ้ำซ้อนกับ (1) ในทางปฏิบัติ
เพราะ bulk import แล้ว verify ต่อในเซสชันเดียวกันเป็นพฤติกรรมจริง ต้องเพิ่ม query logic กันนับซ้ำโดยได้
ประโยชน์ไม่คุ้ม)
→ **หน้า Friends แสดง feed** รวมทุกประเภทเรียงตามเวลาล่าสุดก่อน → **ไม่ mark ว่าอ่านแล้วทันทีตอน mount**
(เร็วเกินไป — เปิดหน้าแล้วสลับแท็บทันทีจะทำให้ของใหม่หายตลอดกาลทั้งที่ยังไม่ได้อ่าน) ต้องหน่วง 2-3 วินาที
ก่อนอัปเดต `feed_last_seen_at = now()` (ยกเลิก timer ถ้า unmount ก่อนครบเวลา — สลับแท็บเร็ว = ไม่ mark)
*AC: user ใหม่/หลัง migrate ไม่เห็นของเก่าทั้งหมดเป็น "ใหม่" พรวดเดียว; badge ขึ้นเฉพาะเมื่อมีของจริงใหม่กว่า
cursor จากทั้ง 3 แหล่ง; badge อยู่ที่ไอคอนแท็บ Friends ไม่ใช่ Dashboard; อยู่ในหน้า Friends ครบ 2-3 วินาที
แล้วเท่านั้น badge จึงหาย, สลับแท็บออกก่อนครบเวลา badge ต้องยังอยู่ (ทดสอบ race condition นี้โดยเฉพาะ);
ไม่มี push/local notification ใดๆ ทั้งฟีเจอร์; อาหาร/จานทุกรายการแสดงชื่อผู้สร้าง; feed แสดงรวมกันไม่เกิน
...
ทันทีในหน้าตัวเอง ไม่ต้องรอ approve → เจ้าของโพสต์แก้/ลบโพสต์ตัวเองได้ (inline, pattern เดียวกับ Diary entry
edit) — **แก้โพสต์ไม่กระทบ `created_at`** ที่ใช้ตัดสิน "ใหม่/เก่า" (แก้ typo ไม่ควร re-trigger badge ให้
ทุกคน) — ข้อความ URL (ขึ้นต้น `http://`/`https://`) ใน body **auto-link เป็นลิงก์กดได้เปิดแท็บใหม่**
(`target="_blank" rel="noopener noreferrer"`) แต่ไม่มี link preview (แยกเป็น BL-19 ยังไม่ทำ) — **ไม่มี**
รูปภาพ (BL-18 ยกเลิกแล้ว), comment/reaction (BL-17, รอเกณฑ์ BL-09 ส่วนที่ 2), push notification

**[แก้ไข 2026-08-22, พบระหว่าง dogfood]** เดิมออกแบบให้โพสต์เป็นแหล่งที่ 4 ผสานรวมกับอาหาร/จาน/คำขอในลิสต์
เดียวที่จำกัด 15-20 รายการ — **ผิด** เพราะโพสต์เป็นเนื้อหาถาวรที่คาดหวังว่ากลับมาอ่านได้ ในขณะที่อาหาร/จาน/
คำขอเป็น "มีอะไรใหม่" ที่ตั้งใจให้หายไปเมื่อเก่า พอรวมกองเดียว ของที่ไหลเร็ว (bulk import ทีละหลายสิบ
รายการ) ดันโพสต์ตกขอบออกจากลิสต์ไปเลย — **แก้เป็น: แยก 2 ส่วนในหน้า Friends** (1) **"โพสต์"** — เฉพาะ posts
เท่านั้น เป็นลิสต์ถาวรไม่ผูก cursor มี "ดูทั้งหมด" ของตัวเอง (2) **"ความเคลื่อนไหว"** — อาหาร/จาน/คำขอ
เหมือนเดิมทุกอย่าง (จำกัด 15-20 รวมกัน, ผูก `feed_last_seen_at` cursor) — **badge ยังใช้ cursor เดียวเดิม
ครอบคลุมทั้ง 4 แหล่ง (รวมโพสต์ด้วย)** ไม่ต้องรื้อ แค่การ**แสดงผล**ที่แยก ไม่ใช่กลไก badge
*AC: (1) ทุก user โพสต์ได้จากหน้า Friends (`body` required ไม่ว่างเปล่า, ≤2,000 ตัวอักษรบังคับทั้ง DB
constraint และ UI) เห็นโพสต์ตัวเองทันทีในส่วน "โพสต์"; (2) ทุกโพสต์แสดงชื่อผู้เขียน; **(3) เจ้าของโพสต์
แก้/ลบโพสต์ตัวเองได้, admin ลบโพสต์คนอื่นได้ (แก้ไม่ได้), user ทั่วไปแก้/ลบโพสต์คนอื่นไม่ได้ — ต้องทดสอบผ่าน
RLS จริง (จำลอง `set_config('request.jwt.claims', ...)` + `set local role authenticated` แบบเดียวกับที่
ทำกับ `food_requests`) ไม่ใช่แค่ซ่อนปุ่มใน UI** เพราะเป็นตารางแรกที่ user เขียนแล้วคนอื่นอ่านได้ ถ้า policy
พลาดเพื่อนแก้โพสต์กันเองได้; (4) โพสต์ใหม่ทำให้ badge ขึ้นเหมือนแหล่งอื่น แม้จะไม่ได้อยู่ในลิสต์
"ความเคลื่อนไหว" ก็ตาม; (5) แก้ไขโพสต์ที่มีอยู่แล้วไม่ทำให้ badge ขึ้นใหม่ (ใช้ `created_at` ไม่ใช่
`updated_at` ตัดสิน); (6) ขึ้นบรรทัดใหม่ในโพสต์แสดงผลถูกต้อง (preserve line break) แต่ไม่มี markdown/HTML
ใดๆ ถูก render; URL ในโพสต์กลายเป็นลิงก์กดได้เปิดแท็บใหม่; (7) ไม่มี UI comment/reaction ใดๆ; ไม่มี
...
- v1.19 (2026-08-21): **FR-DASH-2 superseded โดย FR-FRIEND-1** (คำสั่งวี) — ย้าย feed + notification badge
  ...
  (โชว์ชื่อผู้สร้างเสมอ), ตัด RPC `get_new_admin_foods` ที่ไม่จำเป็นแล้วออก, ตัดแหล่ง "เพิ่ง verify" ที่เสนอ
  ไว้ตอนแรกออก (ซ้ำซ้อนกับอาหารใหม่ในทางปฏิบัติ)
