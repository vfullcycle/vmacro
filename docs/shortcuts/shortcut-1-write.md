...
คู่มือนี้สำหรับคนที่ได้รับลิงก์ Shortcut นี้จากวี — ติดตั้ง + grant สิทธิ์ครั้งเดียวต่อเครื่อง
ไม่ต้องแก้ logic ข้างในเลย นอกจากวาง token ของตัวเองตอนติดตั้ง (ข้อ 3)
...
2. เปิด Vmacro → **Settings → System** → เลื่อนลงมาส่วน "Apple Health" → กด **"สร้าง Token"** → กด **"คัดลอก"**
   (token จะแสดงครั้งเดียว คัดลอกให้ทันก่อนออกจากหน้านี้)
