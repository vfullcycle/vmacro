...
- **ตอนปิด phase/ตี tag ทุกครั้ง ต้องไล่ตาราง backlog ทั้งตาราง (ไม่ใช่แค่รายการที่กำลังทำ) เช็คว่ามีรายการ
  ไหนที่เงื่อนไข/blocker เดิมผ่านไปแล้วแต่ status column ยังไม่อัปเดต** (2026-08-22 — BL-07 ค้างตั้งแต่
  2026-08-18 ที่เงื่อนไข "รอ core จบ" ผ่านไปแล้ว ไม่มีใครย้อนเช็คจนถึงตอนปิด P4f เพราะ P4d/P4e/P4f ไม่ได้ผ่าน
  ขั้นตอน "วางแผน phase" แบบเป็นทางการเลย — rule ข้อบนที่ผูกกับการวางแผนจึงไม่ทันจับ, กติกาข้อนี้เป็นตาข่าย
  รองที่ผูกกับจังหวะปิด phase แทน ครอบคลุมทั้งสองแบบ)
...
- **Tag:** semver ผูกกับ phase gate — annotated tag พร้อมข้อความอ้าง phase
  - `v0.1.0` = จบ P0, `v0.2.0` = จบ P1, `v1.0.0` = จบ P2 (core ครบ), `v1.1.0` = จบ P3, `v1.2.0` = จบ P4a,
    ...
    events), `v1.7.0` = จบ P4f (achievement badges, BL-21), `v1.8.0` = จบ P5 (เลื่อนอีกครั้ง 2026-08-22 —
    P4f แทรกเข้ามาเป็น feature ตาม semver ไม่ใช่ patch เช่นเดียวกับ P4c/P4d/P4e ก่อนหน้า)
